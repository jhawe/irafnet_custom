  ...
  function(x,
    ...
    out <- list(
      ...
      importance = if (importance)
        matrix(rfout$impout, p, 2,
               dimnames = list(
                 x.col.names,
                 c("%IncMSE", "IncNodePurity")
               ))
      else
        matrix(
          rfout$impout,
          ncol = 1,
          dimnames =
            list(x.col.names, "IncNodePurity")
        ),
      ...
      test = if (testdat) {
        list(
          predicted = structure(rfout$ytestpred,
                                names = xts.row.names),
          mse = if (labelts)
            rfout$msets
          else
...
# --- MAIN function
"iRafNet_permutation" <-
  function(X, W, ntree, mtry, genes.name, perm, threads=1) {
